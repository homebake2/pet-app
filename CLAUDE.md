...
- **OpenAPI types**: `open-api/spec.json` is the source of truth for the API contract. `openapi/types.gen.go` is generated from it (`go generate ./...`) and provides response/error-code types (e.g. `openapi.GetErrorResponse`, `openapi.ErrorCodeEnum`) used by handlers via `handlers/response.go`. When changing request/response shapes, update `open-api/spec.json` first, then regenerate. **Whenever `open-api/spec.json` changes, mirror the change into the frontend's `/Users/oka/pet-mono/frontend/openapi/reference/api.yaml`** — convert JSON → YAML, format only, no semantic edits and no client/type generation there.
...
- **Soft delete**: `pet` rows use `deleted_at` instead of hard deletes; all pet queries must filter `deleted_at IS NULL` (see `database.GetPetByIDAndUserID` vs `GetPetIdDBByIDAndUserID`, which intentionally omits the filter for update/ownership checks).
- **Ownership checks**: pets and events are scoped directly by `pet.user_id` (the authenticated user's id from the JWT, via `requireUserID`) — no `profile` row is required to own or access them. `profile` is a separate, optional entity used only by the `/profile` endpoints (`GetProfileIDByUserID`/`GetProfileByUserID`). Cross-user access to pets/events must be checked at the handler level (e.g. `CheckPetBelongsToUser`) before mutating/returning nested resources like events.
...
- **Any change touching the API contract or handler behavior — a new/changed route, request/response shape, status code, validation rule, or error code — must update existing tests or add new ones (unit in `handlers/*_test.go` and/or integration in `integration/`) in the same change.** Don't leave it for a follow-up. If a handler bug is fixed, add a test that would have caught it (this is exactly how `integration/` found and pinned down the NULL-scan and event-date-format bugs — see git history around the `integration/` package's introduction).
